...
class SaveWillPop extends StatefulWidget {
  const SaveWillPop({Key? key, required this.child, required this.isSaved, this.canLeave, this.onQuit, this.onCancel}) : super(key: key);
  final Widget child;
  final bool isSaved;
...
class _SaveWillPopState extends State<SaveWillPop> {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
        child: widget.child,
        onWillPop: widget.canLeave == false ? () async {
          return false;
        } : _onWillPop
    );
  }
